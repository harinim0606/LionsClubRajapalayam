...
// ─── Hoisted outside Settings to prevent remount on every keystroke ────────────
const PwField = ({ id, label, value, showKey, pwShow, setPwShow, setPwForm, placeholder, error }) => (
  <div>
            ...
            {activeTab === "security" && (
              <motion.div key="security" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                ...
                <form onSubmit={handleChangePassword} className="space-y-5 max-w-md">
                  <PwField id="currentPassword" label="Current Password" value={pwForm.currentPassword} showKey="current" pwShow={pwShow} setPwShow={setPwShow} setPwForm={setPwForm} placeholder="Enter your current password" error={pwErrors.currentPassword} />
                  <div className="h-px bg-gray-100 dark:bg-gray-800" />
                  <PwField id="newPassword" label="New Password" value={pwForm.newPassword} showKey="newPw" pwShow={pwShow} setPwShow={setPwShow} setPwForm={setPwForm} placeholder="Minimum 8 characters" error={pwErrors.newPassword} />
                  ...
                  {pwForm.newPassword && (() => {
                    ...
                    return (
                      <div className="space-y-1.5">
                        ...
                        <p className={`text-xs font-semibold ${strength.score <= 1 ? "text-red-500" : strength.score === 2 ? "text-amber-500" : strength.score === 3 ? "text-blue-500" : "text-green-600"}`}>{strength.label}</p>
                        <ul className="text-xs text-gray-500 dark:text-gray-400 space-y-0.5 mt-1">
                          {[
                            { test: pwForm.newPassword.length >= 8, label: "At least 8 characters" },
                            { test: /[A-Z]/.test(pwForm.newPassword), label: "One uppercase letter" },
                            { test: /[0-9]/.test(pwForm.newPassword), label: "One number" },
                            { test: /[^A-Za-z0-9]/.test(pwForm.newPassword), label: "One special character" }
                          ].map(({ test, label }) => (
                  ...
                  <PwField id="confirmPassword" label="Confirm New Password" value={pwForm.confirmPassword} showKey="confirm" pwShow={pwShow} setPwShow={setPwShow} setPwForm={setPwForm} placeholder="Re-enter new password" error={pwErrors.confirmPassword} />

                  <button type="submit" disabled={pwLoading} className="w-full flex items-center justify-center gap-2 py-3 px-6 bg-[#0A2A5E] hover:bg-[#071D43] dark:bg-[#F4B400] dark:hover:bg-[#D69E00] text-white dark:text-black rounded-xl font-bold transition-all shadow-md disabled:opacity-70">
